...
def cd(disk_name):
    ...
    if disk_name not in all_disk:
        ...
        logger.error(f"user {user_name} try to change disk to {disk_name} failed")
    else:
        disk = disk_name
...
def Loop():
    ...
    while True:
        ...
        #d
            ...
            if user_in in files[disk]:
                ...
                logger.error(f"user {user_name} try to delete file {user_in} failed")
    ...
    version => show black OS version""")
        ...
        #r
            ...
            print("restarting...")
            time.sleep(1)
            logger.info(f"user {user_name} restart")
            start()
        #s
        ...
        #other
            ...
            logger.error(f"user {user_name} try to input {user_in} failed is not command")
            time.sleep(1)
...
if are_first == True:
    ...
    logger.info(f"user {user_name} first login")
    start()
else:
    start()